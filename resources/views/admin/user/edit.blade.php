...
<div class="container-fluid">
    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                <h4 class="page-title">{{__('admin.label.edit')}} {{__('admin.label.user')}}</h4>
                <div class="page-title-right">
                    <ol class="breadcrumb m-0">
                        <li class="breadcrumb-item"><a href="{{route('home')}}">{{__('admin.label.dashboard')}}</a></li>
                        <li class="breadcrumb-item active">{{__('admin.label.edit')}} {{__('admin.label.user')}}</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <form enctype="multipart/form-data" id="user-edit" action="{{route('user.update', $user->id)}}"  method="POST">
      @csrf
      @method('PUT')
    ...
    <div class="row">
        ...
        <div class="col-lg-9">
            <div class="custom-bg-white">
                <div class="row">
                    <div class="col-lg-12">
                        <h3 class="form-title">{{__('admin.label.edit')}} {{__('admin.label.user')}}</h3>
                    </div>
                </div>
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <div class="form-group mb-2">
                            ...
                            <select name="gender" class="form-control select2" id="gender">
                                ...
                                @foreach($genders as $key=>$value)
                                    <option value="{{$key}}" @if(strtolower($user->gender) == $key) selected @endif>{{$value}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group mb-2">
                            ...
                            <select name="user_role" class="form-control select2" id="user_role">
                                ...
                                @foreach($roles as $key=>$value)
                                    <option value="{{$value->id}}" @if ($user->user_role == $value->id) selected @endif>{{$value->name}}</option>
                                @endforeach
                            </select>
